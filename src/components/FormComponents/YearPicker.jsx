...
export const SchoolYearSelector = forwardRef(
  (
    {
      minYear = 2000,
    ...
    const handleClickOutside = (event) => {
      if (schoolYearRef.current && !schoolYearRef.current.contains(event.target)) {
        setIsShowing(false);
        if (required && touched && !selectedYear) {
    ...
    }, [touched, selectedYear, required]);

    // ✅ External trigger function
    useImperativeHandle(ref, () => ({
      triggerValidation: () => {
        if (required && !selectedYear) {
          setTouched(true);
          onError("School year is required");
          return false;
        }
        return true;
      },
        ...
        <div className="position-relative" ref={schoolYearRef}>
          ...
          >
            <div className="d-flex flex-row align-items-center gap-3 font-size-sm">
              <span>{selectedYear === null ? placeholder : selectedYear}</span>
            </div>
            <div>
              <span>
                <Icon
