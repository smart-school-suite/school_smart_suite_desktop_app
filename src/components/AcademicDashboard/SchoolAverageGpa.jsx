import { useGetSchoolAverageGpa } from "../../hooks/academicAnalytics/useGetSchoolAverageGpa";
import RectangleSkeleton from "../SkeletonPageLoader/RectangularSkeleton";
import NumberFlow from "@number-flow/react";
import { Icon } from "@iconify/react";
import { useSelector } from "react-redux";
function SchoolAverageGpa() {
  const {
    data: schoolAverageGpa,
    isLoading,
    error,
  } = useGetSchoolAverageGpa(2026);
  return (
    <>
      {isLoading ? (
        ...
        <div className="d-flex flex-row justify-content-between align-items-center">
          <div className="d-flex flex-row align-items-center gap-2">
            <div className="d-flex flex-column">
              <span className="font-size-sm m-0">Average Gpa</span>
              <FormatNumberComponent
                value={schoolAverageGpa?.data?.average_gpa || 0}
              />
            </div>
...
export default SchoolAverageGpa;

const FormatNumberComponent = ({ value, className = "" }) => {
