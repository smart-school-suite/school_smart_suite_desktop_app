...
import { useUpdateDraftSchoolEvent } from "../../hooks/schoolEvent/useUpdateDraftSchoolEvent";
function UpdateDraftEvent({ handleClose, rowData }) {
  ...
  useEffect(() => {
    if (schoolEventDetails?.data) {
      setFormData((prev) => ({
        ...
        teacher_ids:
          JSON.parse(schoolEventDetails?.data?.audience).teachers?.map(
            (items) => ({
              id: items.teacher_id,
            })
          ) || [],
        school_admin_ids:
          JSON.parse(schoolEventDetails?.data?.audience).admins?.map(
            (items) => ({
              id: items.school_admin_id,
            })
          ) || [],
        student_audience: JSON.parse(
          schoolEventDetails?.data?.audience
        ).students?.map((items) => ({
     ...
     <>
       {
        ...
        >
          <span className="m-0">Update School Event Draft Status</span>
          <span
            className="m-0"
            onClick={() => {
              handleClose();
         ...
         ) :  eventCategoryError || schoolEventDetailsError || eventTagError ? (
            <NotFoundError
            title={
              eventCategories?.response?.data?.errors?.title ||
              schoolEventDetails?.response?.data?.errors?.title ||
              eventTagError?.response?.data?.errors?.title
            }
            description={
              eventCategories?.response?.data?.errors?.description ||
              schoolEventDetailsError?.response?.data?.errors?.description ||
...
export default UpdateDraftEvent;
    ...
    <>
      <div className="h-100 d-flex flex-column align-items-center gap-2">
        <div
          ...
          style={{ height: "5%" }}
        >
          <span className="m-0">Update Draft School Event Content</span>
          <span
            className="m-0"
            onClick={() => {
              handleClose();
...
function CreateSchoolEvent({
  ...
  const { data: schoolAdmins, isLoading: isSchoolAdminLoading } =
    useGetSchoolAdmins();
  const { mutate:updateDraftSchoolEvent, isPending } = useUpdateDraftSchoolEvent(handleClose);
  ...
  const handleCreateSchoolEvent = async () => {
    ...
    updateDraftSchoolEvent(payload);
  };
  return (
    <>
      <div className="h-100 d-flex flex-column align-items-center">
        ...
        >
          <span className="m-0">Update School Draft Event Status</span>
          <span
            className="m-0"
            onClick={() => {
              handleClose();
        ...
        >
          <div className="d-flex flex-column" style={{ width: "70%" }}>
            ...
            <div>
              ...
              <CustomDropdown
                data={announcementStatus.filter((items) => items.name !== "draft") || []}
              ...
              <div className="w-100 mt-1">
                ...
                <div className="w-100">
                  ...
                  <DateTimeInput
                    ...
                    value={truncateSeconds(formData.value)}
