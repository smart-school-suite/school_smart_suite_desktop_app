...
import { useDeleteExamTimetableMutation } from "../../Slices/Asynslices/deleteSlice";
import toast from "react-hot-toast";
import ToastSuccess from "../../components/Toast/ToastSuccess";
function DeleteTimetable({ handleClose, row_id:examId }){
    const [isDeleting, setIsDeleting] = useState(false);
    const [deleteExamTimetable] = useDeleteExamTimetableMutation();
    const handleDeleteTimetable = async () => {
         try{
             await deleteExamTimetable(examId).unwrap();
             handleClose();
             setIsDeleting(false);
             toast.custom(
                <ToastSuccess 
             ...
             toast.custom(<ToastDanger 
                title={"Delete Failed ❌"}
                description={"❌ Something went wrong! The Exam time table delete failed due to an error. Please try again later."}
              />);
         }
    }
    return(
