...
import { useDeleteResitScores } from "../../hooks/resitScore/useDeleteResitScores";
function DeleteResitScores({ handleClose, rowData }) {
  const { id: candidateId } = rowData;
  const { mutate: deleteResitScores, isPending } = useDeleteResitScores(
    handleClose,
    candidateId,
  );
  const handleDeleteScores = () => {
    deleteResitScores(candidateId);
  };
  return (
    <>
      <div className="w-100">
        ...
        >
          <div className="d-flex flex-row align-items-center justify-content-between">
            <div>
              <span className="font-size-sm fw-semibold">Delete Resit Scores</span>
            </div>
            <button
              onClick={() => handleClose()}
...
export default DeleteResitScores;
