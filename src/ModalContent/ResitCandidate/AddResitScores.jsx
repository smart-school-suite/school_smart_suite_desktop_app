...
function AddResitScores({ handleClose, rowData }) {
  const moduleState = useSelector((state) => state.resitEvaluation);
  ...
  const {
    ...
    isLoading,
    error,
  } = useGetResitEvaluationHelperData(candidateId);

  const { mutate: createResitScore, isPending } =
    useCreateResitScore(handleClose);
  useEffect(() => {
  ...
  const handleAddResitScores = () => {
    const payload = {
      candidate_id: candidateId,
      scores: moduleState?.scores?.map((score) => ({
        resit_id: score.resit_id,
        score: score.score,
      })),
    };
    createResitScore(payload);
  };
  return (
    <>
            ...
            <div className="d-flex flex-column gap-1">
              <div className="d-flex flex-row justify-content-between align-items-center">
                ...
                <small style={{ fontSize: "0.7rem" }} className="fw-semibold">
                  {parseFloat((coursesEvaluated / moduleState?.scores?.length)).toFixed(2) * 100} %
                  complete
                </small>
              </div>
              <ProgressBar
              ...
              <div className="d-flex flex-row justify-content-between align-items-center w-100">
                ...
                <span
                  className={`fw-semibold font-size-sm ${
                    moduleState?.resultSummary?.examStatus.toLowerCase() ==
                    RESULT?.PASSED
                      ? "text-fern-500"
                      : moduleState?.resultSummary?.examStatus.toLowerCase() ==
                          RESULT.FAILED
