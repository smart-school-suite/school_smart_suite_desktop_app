import { useDeleteResitFeeTransactionMutation } from "../../Slices/Asynslices/deleteSlice";
...
function DeleteTransaction({ row_id: transactionId, handleClose }) {
  ...
  const [deleteResitFeeTransaction] = useDeleteResitFeeTransactionMutation();

  const handleDeleteTuitionFeeTransaction = async () => {
    setFeedback({ message: "", type: null, loading: true });
    try {
      await deleteResitFeeTransaction(transactionId).unwrap();
      setFeedback({
