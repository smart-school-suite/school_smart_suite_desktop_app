...
import { useActivateSchoolAdmin } from "../../hooks/schoolAdmin/useActivateSchoolAdmin";
import { CircleX } from "lucide-react";
function ActivateSchoolAdmin({ handleClose, rowData }) {
  const { id: schoolAdminId } = rowData;
  const { mutate: activate, isPending } = useActivateSchoolAdmin();
  const handleActivate = () => {
    activate(schoolAdminId);
  };
  return (
    <>
      <div className="w-100">
        ...
        >
          <div className="d-flex flex-row align-items-center justify-content-between">
            <div>
              <span className="font-size-sm fw-semibold">Activate School Admin</span>
            </div>
            <button
              onClick={() => handleClose()}
...
export default ActivateSchoolAdmin;
