...
import { useDeleteExamScores } from "../../hooks/examScore/useDeleteExamScores";
function DeleteExamScores({ handleClose, rowData }) {
   const { id: candidateId } = rowData;
  const { mutate: deleteExamScores, isPending } =  useDeleteExamScores(
    handleClose,
    candidateId,
  );
  const handleDeleteScores = () => {
    deleteExamScores(candidateId);
  };
  return (
    <>
      <div className="w-100">
...
export default DeleteExamScores;
