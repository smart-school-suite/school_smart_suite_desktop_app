...
import { useDeleteCaExamScores } from "../../hooks/examScore/useDeleteCaScores";
function DeleteCaScores({ handleClose, rowData }) {
  const { id: candidateId } = rowData;
  const { mutate: deleteCaExamScores, isPending } = useDeleteCaExamScores(
    handleClose,
    candidateId,
  );
  const handleDeleteScores = () => {
    deleteCaExamScores(candidateId);
  };
  return (
    <>
      <div className="w-100">
