...
import { useDeleteBatch } from "../../hooks/studentBatch/useDeleteBatch";
function DeleteStudentBatch({ handleClose, rowData }) {
  const batchId = rowData.id;
  const { mutate: deleteBatch, isPending } = useDeleteBatch(handleClose);
  const handleDelete = () => {
    deleteBatch(batchId);
  };
  return (
    <div className="w-100">
      ...
      >
        <div className="d-flex flex-row align-items-center justify-content-between">
          <div>
            <span className="font-size-sm fw-semibold">Delete Department</span>
          </div>
          <button
            onClick={() => handleClose()}
