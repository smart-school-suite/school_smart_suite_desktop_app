...
function ImportColumnMatch({
  ...
  const moduleColumns = useMemo(() => moduleCols, []);
  const standardColumns = useMemo(
    () => moduleColumns.filter((c) => c.type === "standard_field"),
    [moduleColumns],
  );
  ...
  const totalFields = useMemo(() => moduleColumns.length, [moduleColumns]);
  ...
  const missingRequiredFields = useMemo(() => {
    return moduleColumns.filter(
      (col) => col.required && !mapping[col.program_name],
    );
  }, [moduleColumns, mapping]);
  ...
  }, [moduleColumns, mapping, fileHeaders]);
          ...
          <div className="d-flex flex-column gap-2">
            ...
            >
              {moduleColumns
                ...
                .map((column) => {
                  const selectedValue =
                    moduleState?.import?.mapping?.standardFields[
                      column?.program_name
                    ]?.value;
                  return (
                    <>
        ...
        { moduleState?.import?.mapping?.repeatableGroups && (
          <>
            ...
            >
              ...
              >
                {Object.keys(moduleState.import.mapping.repeatableGroups).map(
                  (rG) => (
                    <Fragment key={rG}>
...
function RepeatableGroupAccordion({ rG, moduleState, fileHeaders, moduleInstanceMap }) {
  ...
  const rgInstance = moduleInstanceMap.find((i) => i.key.toLowerCase() === rG);
  return (
    <div
