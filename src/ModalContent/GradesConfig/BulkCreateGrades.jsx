...
import { useBulkCreateGradesByCategory } from "../../hooks/schoolGradeCategory/useBulkCreateGradesByCategory";
import { resetAll } from "../../Slices/Asynslices/AutoGenTimetableSlice";
function BulkCreateGrades({ handleClose, resetAll, bulkData }){
...
export default BulkCreateGrades;
...
function ExamGradingPreview({ handleClose, handleStateChange, maxScore, rowData, bulkData }){
   const [genGrades, setGenGrades] = useState([]);
   const { mutateAsync:generateGrades, isPending } = useAutoGenExamGrades();
   const { mutate:createGrades, isPending:isCreating } = useBulkCreateGradesByCategory(handleClose, resetAll);
   ...
   const handleCreateGrades = async () => {
      ...
      const gradeCategories = bulkData.map((items) => ({
          grade_config_id:items.id
      }))
      createGrades({ grades:gradesData, configIds:gradeCategories })
   }
   useEffect(() => {
      if(maxScore > 0){
         handleGradesGen();
