...
export const bulkDeleteExam = async (examIds) => {
  const response = await axiosInstance.post("exam/bulk-delete", {
    ids: examIds,
  });
  return response.data;
};
...
export const getRelatedCaExams = async (schoolYearId, examTypeId) => {
  const response = await axiosInstance.get(
    `exam/related-ca/school-year/${schoolYearId}/exam-type/${examTypeId}`,
  );
  return response.data;
};
