...
import { useGetDraftSchoolEvents } from "../../hooks/schoolEvent/useGetDraftSchoolEvent";
import { useGetDraftSchoolEventsByCategory } from "../../hooks/schoolEvent/useGetDraftSchoolEventsByCategory";
import UpdateDraftEvent from "../../ModalContent/Events/UpdateDraftEvent";
          ...
          <div className="mt-auto">
            <div className="d-flex flex-row w-100 align-items-end justify-content-between">
              <div className="d-flex flex-column gap-2">
                ...
                <div className="d-flex flex-column">
                    <span className="font-size-xs gainsboro-color">Event Duration</span>
              ...
              <div className="d-flex flex-row gap-2 align-items-center pe-2">
               <ModalButton
                    action={{ modalContent: UpdateDraftEvent }}
                    fullscreen={true}
                    rowData={data}
               >
                 <button className="border-none font-size-sm rounded-3 px-3 py-2 fw-medium d-flex gap-2"
                  style={{ background:"#a572da", padding:"0.7rem", color:"#fafafa" }}
                >
...
function AllEvents() {
  const { data: schoolEvents, isLoading, error } = useGetDraftSchoolEvents();
  return (
    <>
      {isLoading ? (
...
function CategorizedEvents({ categoryId }) {
  const {
    ...
    isLoading,
    error,
  } = useGetDraftSchoolEventsByCategory(categoryId);
  return (
    <>
      {isLoading ? (
        <div className="d-flex flex-column gap-2 w-100">
