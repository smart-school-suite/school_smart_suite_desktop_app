...
import { resitColDefs } from "../../utils/table/colDefs/resit/resitColDefs";
...
import ResitDetails from "../../DrawerContent/Resit/ResitDetails";
function StudentResit() {
  const { data: studentResits, isLoading, error } = useGetStudentResits();
  const darkMode = useSelector((state) => state.theme.darkMode);
  const dispatch = useDispatch();
  const tableWrapperRef = useRef(null);
  const resitState = useSelector((state) => state.resit);
              ...
              <div className="h-100">
                <div className="d-flex flex-row align-items-start w-100 h-100 gap-1">
                  <motion.div
                    ...
                    style={{
                      width: resitState.isGeneralFilterOpen ? "60%" : "100%",
                    }}
                  >
                    <Table
                  ...
                  {resitState.isGeneralFilterOpen && (
                      ...
                      {resitState.isGeneralFilterOpen && (
                        ...
                        >
                          ...
                          >
                            <div className="d-flex flex-row align-items-center justify-content-between">
                              <span>
                                Build a custom view of your Resit data.
                              </span>
                              <button
                            ...
                            <div className="d-flex flex-row align-items-center justify-content-between">
                              <div className="d-flex flex-row align-items-center gap-2">
                                ...
                                <span>Filter Exam Candidates</span>
                              </div>
                              <span>{studentResits?.data.length} items</span>
                            </div>
                          </div>
                          <div
                          ...
                          >
                            {resitState.customFilter.length > 0 ? (
                              <div>
                                {resitState?.customFilter?.map((cFilters) => (
                                  <Fragment key={cFilters.id}>
                                    <GeneralFilterWizzard
                                      cFilters={cFilters}
                                      columns={columns}
                                      moduleState={resitState}
    ...
    <>
      ...
      >
        <DropDownMenuItem
          ...
          onClick={() =>
            handleShowModal(DeleteStudentResit, {
              size: "md",
              closeOnOutsideClick: true,
              closeOnEscape: true,
            })
