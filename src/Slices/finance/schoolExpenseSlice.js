...
const schoolExpenseSlice = createSlice({
  name: "schoolExpense",
  initialState,
  reducers: {
    ...
    setSelectedschoolExpenses: (state, action) => {
      state.selectedSchoolExpenses = action.payload;
    },
...
} = schoolExpenseSlice.actions;

export default schoolExpenseSlice.reducer;

// Selectors
export const selectschoolExpenses = (state) => state.schoolExpenses.schoolExpenses;
export const selectschoolExpensesData = (state) =>
  state.schoolExpenses.schoolExpenses?.data ?? [];
export const selectIsLoading = (state) => state.schoolExpenses.isLoading;
export const selectError = (state) => state.schoolExpenses.error;
export const selectSelectedschoolExpenses = (state) =>
  state.schoolExpenses.selectedSchoolExpenses;
export const selectRowCount = (state) => state.schoolExpenses.rowCount;
export const selectSearchText = (state) => state.schoolExpenses.searchText;
export const selectColumns = (state) => state.schoolExpenses.columns;
export const selectAvailableColumns = (state) =>
  state.schoolExpenses.columns.availableColumns;
export const selectSelectedColumns = (state) =>
  state.schoolExpenses.columns.selectedColumns;

export const selectSchoolExpenseCount = createSelector(
  [selectschoolExpensesData],
  (schoolExpenses) => schoolExpenses?.length || 0,
);

export const selectFilteredschoolExpenses = createSelector(
  [selectschoolExpensesData, selectSearchText],
  (schoolExpenses, searchText) => {
    if (!searchText) return schoolExpenses;
    return (
      schoolExpenses?.filter((teacher) =>
