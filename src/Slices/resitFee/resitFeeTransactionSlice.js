...
const initialState = {
  resitFeeTransaction: null,
  isGeneralFilterOpen: false,
  tableRef: null,
  selectedResitTransaction: [],
  rowCount: 0,
  searchText: "",
  columns: {
    selectedColumns: [],
...
const resitFeeTransactionSlice = createSlice({
  name: "resitFeeTransaction",
  initialState,
  reducers: {
    ...
    setSelectedResitTransactions: (state, action) => {
      state.selectedResitTransaction = action.payload;
    },
    ...
    resetSelections: (state) => {
      state.selectedResitTransaction = [];
      state.rowCount = 0;
    },
    resetAll: (state) => {
      state.selectedResitTransaction = [];
...
export const {
  ...
  setImportReset,
  setColumnMapping
} = resitFeeTransactionSlice.actions;

export default resitFeeTransactionSlice.reducer;
